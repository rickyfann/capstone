...
def costing_comparison(q_array, cleanings):
    # total operation days * cost of operation per day + downtime days * loss per downtime day
    return (3650 - 5 * cleanings) * cost(np.average(q_array)) + 5 * cleanings * 200000

def func(par):
    f_threshold, q_threshold = par
    x_vals = np.linspace(0, 3650, 1000)
    y_vals = f(t = x_vals)
    q_vals = q(t = x_vals)
    check_index = []
    ...
    return costing_comparison(q_vals, max(len(check_index), len(check_index2)))

minimum = fmin(func, [0.9, 3650])